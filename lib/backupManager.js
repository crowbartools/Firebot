        ...
        fs.readdir(backupFolderPath, (err, files) => {
            let fileNames =
                files.map(function(v) {
                    ...
                    return !n.includes("NODELETE");
                });

            fileNames.splice(0, maxBackups);

            fileNames.forEach(f => {
                console.log("deleting old backup: " + f);
...
function startBackup(manualActivation = false, callback) {
    ...
    let version = app.getVersion(),
        formatedTime = moment().format('[d]YYYY-MM-DD-[t]hh-mm'),
        milliseconds = Date.now(),
        fileExtension = 'zip';

    let filename = `backup-${milliseconds}-v${version}-${formatedTime}${manualActivation ? "-manual" : ""}.${fileExtension}`;
...
function onceADayBackUpCheck() {
    ...
    if (shouldBackUp) {
        ...
        if (!isSameDay) {
            console.log("Doing once a day backup");
            startBackup();
        }
    }
}
