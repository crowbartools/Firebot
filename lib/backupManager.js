const settings = require('./interactive/settings-access').settings;
const dataAccess = require('./data-access.js');
const path = require('path')
const fs = require('fs')

function startBackupManager(manualActivation = false){
    ...
    output.on('close', function() {
        ...
        if(manualActivation) renderWindow.webContents.send('info', "The backup has been finished, you can close this message safely. (Backup size: "+ filesize +" bytes)");
    });

    archive.on('warning', function(err) {
        if (err.code === 'ENOENT') {
            // log warning
            if(manualActivation) renderWindow.webContents.send('error', "There was an error initiating the backup function.");
...
exports.startBackupManager = startBackupManager;