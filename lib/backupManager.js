...
let backupFolderPath = path.resolve(dataAccess.getPathInUserData('/') + path.sep + "backups") + path.sep;
...
function startBackup(manualActivation = false, callback) {
    ...
    try {
        let streamerDb = profileManager.getJsonDbInProfile('./auth');
        streamerName = streamerDb.getData('/streamer/username');
    } catch (err) {
        logger.warn(err);
    }

    let filename = `backup_${streamerName}_${milliseconds}_v${version}${manualActivation ? "_manual" : ""}.${fileExtension}`;
    ...
    let folderPath = path.resolve(profileManager.getPathInProfile('/'));
