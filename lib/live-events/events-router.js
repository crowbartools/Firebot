...
function liveEventRouter (event) {
    ...
    let events = Object.values(eventSettingsCache);

    let eventSettings = events.find(e => e.active === true && e.eventType === event.type);

    if (eventSettings == null || eventSettings.effects == null) {
        return;
    }

    effects = eventSettings.effects;
