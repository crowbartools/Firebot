...
function cacheEvent(sourceId, eventId, eventMetaKey = null) {
  /**
...
function liveEventRouter(event) {
  ...
  Object.keys(eventsGroup["events"]).forEach(k => {
    eventSetting = eventsGroup["events"][k];
    if (eventSetting.active === true && eventSetting.type === event.type) {
...
ipcMain.on("manualEvent", function(event, eventSettings) {
  manualEvent(eventSettings.type);
});
