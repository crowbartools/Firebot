        ...
        if (data && data.manifest) {
            ...
            if (data.manifest.name === "giphy") {
                ...
                if (data.parameters && data.parameters.gifUrl) {
                    ...
                    logger.debug("Getting channeldata for id " + userId + " so we can trigger gif event");

                    apiAccess.get(`channels/${userId}`)
                        .then(channelData => {

                            logger.debug("channel data", channelData);

                            logger.debug("Got channel data, triggering gif event with url: " + data.parameters.gifUrl);

                            let event = new LiveEvent(EventType.SKILL_GIF, EventSourceType.CONSTELLATION, {
                                username: channelData ? channelData.token : "Unknown",
                                gifUrl: data.parameters.gifUrl
                            });
