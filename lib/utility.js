...
exports.populateStringWithTriggerData = async function(string = "", trigger) {
    ...
    if (trigger.type === TriggerType.INTERACTIVE) {
        ...
        if (trigger.metadata.eventData.previousPatronageData) {
            let patronageData = trigger.metadata.eventData.previousPatronageData;
            if (patronageData.channel && patronageData.period) {

                let milestoneGroup = patronageData.period.milestoneGroups.find(mg => mg.id === patronageData.channel.currentMilestoneGroupId);
                if (milestoneGroup) {
                    let currentMilestone = milestoneGroup.milestones.find(m => m.id === patronageData.channel.currentMilestoneGroupId);
                    replaceDict["$(patronagePreviousMilestoneTarget)"] = currentMilestone.target;
                    replaceDict["$(patronagePreviousMilestoneReward)"] = currentMilestone.reward / 100;
                }
            }
        }
    }
