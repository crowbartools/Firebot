...
exports.populateStringWithTriggerData = async function(string = "", trigger) {
    ...
    if (trigger.type === TriggerType.INTERACTIVE) {
        ...
        replaceDict["$(activeState)"] = control.enabled ? "enabled" : "disabled";
        replaceDict["$(activeStateReverse)"] = control.enabled ? "disabled" : "enabled";
    ...
    if (string.includes("$(math[")) {
        ...
        for (let mathData of mathMatches) {
            ...
            try {
                ...
                logger.warning("error parsing math expression", err);
                evalulation = "[MATH PARSE ERROR]";
            }
            replaceDict2[mathData.full] = evalulation;
        }
    }
            ...
            if (apiCall.jsonPathNodes != null) {
                if (content != null) {
                    try {
                        ...
                        replaceDict2[apiCall.full] = currentObject ? currentObject.toString() : "";
                    } catch (err) {
                        logger.warning("error when parsing api json", err);
