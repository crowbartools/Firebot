...
exports.populateStringWithTriggerData = async function(string = "", trigger) {
    ...
    if (string.includes("$(costreamChannels)")) {
        ...
        try {
            ...
            if (streamerChannel.costreamId != null) {
                ...
                if (costream && costream.channels) {
                    ...
                    replaceText = names.join(", ");
                } else {
                    replaceText = "[No Costream Found]";
                }
            } else {
                replaceText = "[No Costream Found]";
            }
        ...
        replaceDict["$(costreamChannels)"] = replaceText;
    }
    ...
    if ((trigger.type === TriggerType.EVENT || trigger.type === TriggerType.MANUAL) && trigger.metadata.eventData) {
        ...
        if (eventData.gifterUsername) {
            replaceDict["$(gifterUsername)"] = eventData.gifterUsername;
        }
        if (eventData.giftReceiverUsername) {
            replaceDict["$(giftReceiverUsername)"] = eventData.giftReceiverUsername;
        }
    ...
    return populateStringWithReplaceDict(string, replaceDict5);
};
