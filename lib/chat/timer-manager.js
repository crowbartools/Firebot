...
function buildIntervalsForTimers(timers, onlyClearWhenLiveTimers = false) {
  ...
  for (let timer of timers) {
    ...
    if (timer.onlyWhenLive && !connectionManager.isOnline()) continue;
    /**
     * Create the interval.
...
function startTimers() {
  ...
  let timers = commandAccess.getTimers().filter(t => t.active);
