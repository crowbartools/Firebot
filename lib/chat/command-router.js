...
function userIsInRole(userRoles, approvedRoles) {
    if (approvedRoles.length === 0 || approvedRoles === null || approvedRoles === undefined) {
        return true;
    }
    ...
    for (let command in activeCommands) {
        if (activeCommands.hasOwnProperty(command)) {
            ...
            if (userCommand['cmd'].value === trigger) {
                ...
                if (command.permissionType === "Group" || command.permissionType === undefined || command.permissionType === null) {
