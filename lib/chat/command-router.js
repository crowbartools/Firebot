...
function handleChatCommand(chatEvent, chatter, firebot) {
 ...
 // If it does, that means that one of the logged in accounts has already handled the message.
  ...
  for (command in activeCommands){
    ...
    if(userCommand['cmd'].value == trigger){
      ...
      // Check to see if the command is on cooldown or not.
      if( cooldownChecker(command.commandID, command.cooldown) === true){
        // Command is off of cooldown!
        ...
        if(Object.prototype.toString.call( command.permissions ) === '[object Array]' && command.permissions.length > 0){
          ...
          if(!userHasPermission && chatter.toLowerCase() !== 'streamer') { 
            ...
            processChatEffects(commandSender, isWhisper, command, firebot, chatEvent, userCommand, false);
            return;
          }
...
function processChatEffects(commandSender, isWhisper, command, firebot, chatEvent, userCommand, timedCmd){
