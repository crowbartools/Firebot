...
THIS IS THE OLD V4 COMMAND LOGIC AND WILL BE PHASED OUT. NO NEED TO LOOKTHROUGH OR UPDATE THIS PAGE. REFER TO
"commandRouter" INSTEAD.
...
function processChatEffects(
  ...
  if (command.skipLog !== true) {
    renderWindow.webContents.send("eventlog", {
      type: "general",
      username: commandSender,
      event: "used the " + command.commandID + " command."
    });
  }
  ...
  if (command.chatFeedAlert === true) {
    renderWindow.webContents.send("chatUpdate", {
      fbEvent: "ChatAlert",
      message: commandSender + " used the " + command.commandID + " command."
    });
  }
    ...
    if (activeCommands.hasOwnProperty(command)) {
      ...
      if (userCommand["cmd"].value === trigger) {
        ...
        if (command.permissionType === "Group") {
          ...
          let userHasPermission = userIsInRole(
            getCombinedRoles(commandSender, chatEvent),
            mapRoleNames(command.permissions)
          );

          logger.debug(
            "user has permission to run command: " + userHasPermission
          );
          ...
          if (
            cooldownChecker(command.commandID, command.cooldown) === true ||
            commandSender === streamerName
          ) {
          ...
          if (
            commandSender === command.permissions ||
            command.permissions === ""
          ) {
            // User has permission.
            if (
