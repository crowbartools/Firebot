  ...
  if (savedCommand.commands.length > 0) {
    ...
    if (group.randomize === true) {
      ...
      let randomNumber = Math.floor(
        Math.random() * savedCommand.commands.length
      );
    ...
    if (command != null && testArray.indexOf(cmdToFire) !== -1) {
      ...
      logger.info(
        "Timed Command list had an inactive command (" +
          cmdToFire +
          ") in it. Cleaning up the list a trying again."
      );
      let dbCommands = profileManager.getJsonDbInProfile("/chat/commands");
      let commandList = dbCommands.getData(
        "/timedGroups/" + groupName + "/commands"
      );
