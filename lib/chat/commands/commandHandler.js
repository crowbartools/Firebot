...
function checkForCommand(rawMessage) {
  ...
  let activeSystemCommands = systemCommandManager
      .getSystemCommands()
      .map(c => {
        let cmd = c.definition;
        cmd.type = "system";
        return cmd;
      })
      .filter(c => c.active),
    activeCustomCommands = commandAccess
      .getCustomCommands()
      .filter(c => c.active),
    allCommands = activeSystemCommands.concat(activeCustomCommands);

  console.log(activeSystemCommands);
...
function fireCommand(
  ...
  if (command.type === "system") {
    console.log("firing system command");
    //get system command from manager
    let cmdDef = systemCommandManager.getCommandById(command.id);
...
function triggerCustomCommand(id, isTimed, isManual) {
  let command = commandAccess.getCustomCommand(id);
  console.log("firing command man", command);
  fireCommand(command, null, null, null, isTimed, isManual);
}

// Refresh command cooldown cache when changes happened on the front end
ipcMain.on("commandManualTrigger", function(event, id) {
  triggerCustomCommand(id, null, true);
});
