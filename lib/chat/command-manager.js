...
function checkForCommand(rawMessage) {
  ...
  let activeSystemCommands = commandAccess
      .getSystemCommands()
      .filter(c => c.active),
...
function manuallyTriggerCustomCommand(id) {
  let command = commandAccess.getCustomCommands().find(c => c.id === id);
  if (command == null) return;

  let userCommand = buildUserCommand(
    command,
    "",
    accountAccess.getAccounts().streamer.username
  );

  customCommandExecutor.execute(command, userCommand, {}, false, true);
}

// Refresh command cooldown cache when changes happened on the front end
ipcMain.on("commandManualTrigger", function(event, id) {
  console.log(id);
  manuallyTriggerCustomCommand(id);
});
