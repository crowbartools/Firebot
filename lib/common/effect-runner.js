...
function runEffect(effect, trigger) {
    ...
    case EffectTypeList.CREATE_CLIP:
        if (true) break; // eslint-disable-line no-constant-condition
        Chat.createClip(effect.clipDuration).then(success => {
            if (success) {
                logger.info("Successfully created a Mixer clip!");
            } else {
                renderWindow.webContents.send('error', "Failed to create a clip on Mixer. Make sure you are live and try again.");
            }
        });
        break;
