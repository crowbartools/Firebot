...
function runEffects(runEffectsContext, callback) {
    ...
    if (triggerType === TriggerType.COMMAND) {
        ...
        let userCommand = runEffectsContext.userCommand;
        let isWhisper = runEffectsContext.isWhisper;
        let chatEvent = runEffectsContext.chatEvent;
    } else {
    // interactive, custom scripts, and api all could be using a participant
        let participant = runEffectsContext.participant;
    }
