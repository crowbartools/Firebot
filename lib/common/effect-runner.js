...
function runEffects(runEffectsContext, callback) {
    ...
    if (triggerType === TriggerType.COMMAND) {
        let command = runEffectsContext.command;
    ...
    // For each effect, send it off to the appropriate handler.
        ...
        case EffectTypeList.CUSTOM_SCRIPT:
            try {
                ...
                if (!isManual) {
                    customScriptHandler.processScript(effect.scriptName, effect.parameters, control, participantObj, firebot, userCommand);
                } else {
                    customScriptHandler.processScript(effect.scriptName, effect.parameters, control, {username: 'Test Username'}, firebot, userCommand);
                }
            } catch (err) {
        ...
        case EffectTypeList.RUN_COMMAND:
            var commandsCache = Chat.getCommandCache();

            var savedCommand = commandsCache.Active[effect.id];
