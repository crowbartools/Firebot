...
function validateEffectCanRun(effectType, triggerType) {
    ...
    if (!effectDefinition.triggers.includes(triggerType)) {
        console.warn(`${effectType} cannot be triggered by: ${triggerType}`);
        return false;
    }
    ...
    let validDeps = effectDefinition.dependencies.every(d => {
        ...
        console.warn(`Unknown effect dependancy: ${d}`);
        return false;
    });
...
function runEffect(effect, trigger) {
    ...
    default:
        ...
        console.log('Oops! This effect type doesnt exist: ' + effect.type);
    }
}
...
function runEffects(runEffectsContext, callback) {
    ...
    if (validateEffectCanRun(effect.type, trigger.type)) {
        ...
        console.log('Skipping ' + effect.type + '. Dependencies not met or trigger not supported.');
        renderWindow.webContents.send('eventlog', {username: "System:", event: `Skipped over ${effect.type} due to dependencies or unsupported trigger.`});
