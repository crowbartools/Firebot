    ...
    return new Promise(async resolve => {
        ...
        for (let key of keys) {
            ...
            if (value && typeof value === "object") {
                // recurse
                await findAndReplaceVariables(data, trigger);
            }
    ...
    return new Promise(async resolve => {
        ...
        for (const effect of effects) {
            ...
            if (!validateEffectCanRun(effect.id, trigger.type)) {
                logger.info("Skipping " + effect.id + ". Dependencies not met or trigger not supported.");
            ...
            try {
                ...
                if (response && response.success === false) {
                    logger.error(`An effect with ID ${effect.id} failed to run.`, response.reason);
                }
            } catch (err) {
                logger.error(`There was an error running effect with ID ${effect.id}`, err);
            }
        }
