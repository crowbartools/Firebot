...
function runEffect(effect, trigger) {
  ...
  switch (effect.type) {
    ...
    case EffectTypeList.CUSTOM_SCRIPT:
      try {
        ...
        renderWindow.webContents.send(
          "error",
          "Oops! There was an error processing the custom script. Error: " + err
        );
        logger.error(err);
      }
      break;
    ...
    default:
      renderWindow.webContents.send(
        "error",
        "Oops! This effect type doesnt exist: " + effect.type
      );
...
function runEffects(runEffectsContext, callback) {
  ...
  let effectsCount = effects.length;

  if (effectsCount < currentIndex + 1) {
    callback();
    return;
  }
  ...
  if (runResult.waitingOnOtherEffects != null) {
    runResult.waitingOnOtherEffects.then(() => {
      // eslint-disable-next-line no-use-before-define
      delayNextEffectRun(
        runResult.delayBeforeNextEffect,
        runEffectsContext,
        callback
      );
    });
...
function processEffects(processEffectsRequest) {
  return new Promise(resolve => {
    ...
    runEffectsContext["previousIndex"] = -1;
