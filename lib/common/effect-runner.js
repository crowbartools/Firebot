...
const shoutoutHandler = require('./handlers/shoutoutProcessor.js');
...
function runEffects(runEffectsContext, callback) {
    ...
    if (isManual && (username == null || username !== '')) {
    ...
    // For each effect, send it off to the appropriate handler.
        ...
        case EffectTypeList.HTML:
            ...
            break;
        case EffectTypeList.SHOUTOUT:
            if (!isManual || isApi) {
                shoutoutHandler.go(effect, {username: username}, control);
            } else {
                shoutoutHandler.go(effect, {username: 'Streamer'}, control);
            }
            break;
        ...
        case EffectTypeList.TEXT_TO_FILE:
            console.log("heren");
            fileWriter.run(effect, triggerType, username, control, null, userCommand);
            break;
        ...
        case EffectTypeList.RANDOM_EFFECT: {
            ...
            let effectList;
            if (effectType === EffectTypeList.RANDOM_EFFECT) {
            ...
            let processEffectsRequest = {
                type: runEffectsContext.type,
                effects: EffectBuilder.buildEffects(effectList),
                firebot: runEffectsContext.firebot,
                participant: runEffectsContext.participant,
                control: runEffectsContext.control,
                isManual: runEffectsContext.isManual
            };

            // eslint-disable-next-line no-use-before-define
            waitingOnOtherEffects = processEffects(processEffectsRequest);
            break;
        }
        case EffectTypeList.CUSTOM_SCRIPT:
            try {
                ...
                if (!isManual) {
                    customScriptHandler.processScript(effect.scriptName, effect.parameters, control, userCommand, participantObj, firebot, triggerType);
                } else {
                    customScriptHandler.processScript(effect.scriptName, effect.parameters, control, userCommand, {username: 'Test Username'}, firebot, triggerType);
                }
            } catch (err) {
    ...
    // Run the next effect
    if (waitingOnOtherEffects != null) {
        waitingOnOtherEffects.then(() => {
            // eslint-disable-next-line no-use-before-define
            delayNextEffectRun(delayBeforeNextEffect, runEffectsContext, callback);
        });
    } else {
        // eslint-disable-next-line no-use-before-define
        delayNextEffectRun(delayBeforeNextEffect, runEffectsContext, callback);
...
function delayNextEffectRun(delayBeforeNextEffect, runEffectsContext, callback) {
    // Run the next effect
