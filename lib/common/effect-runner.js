...
const showTextHandler = require('./handlers/showTextProcessor.js');
...
function runEffects(runEffectsContext, callback) {
    ...
    // For each effect, send it off to the appropriate handler.
        ...
        case EffectTypeList.SHOW_TEXT:
            if (!isManual || isApi) {
                showTextHandler.go(effect, {username: username}, control);
            } else {
                showTextHandler.go(effect, {username: 'Streamer'}, control);
            }
            break;
