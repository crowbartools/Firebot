...
function clearReconnectTimeout(connectionType) {
  ...
  switch (connectionType) {
    ...
    default:
      logger.error(
        "Invalid connection type passed to clear reconnect timeout."
      );
      return;
  }
}
...
function chatReconnect() {
  ...
  if (!chatConnected && retry <= maxRetries) {
    ...
    renderWindow.webContents.send(
      "error",
      "Chat reconnection failed several times. Stopping reconnection attempts."
    );
    logger.error(
      "Chat connection failed several times. Stopping reconnect attempts."
    );
...
function interactiveReconnect() {
  ...
  if (!interactiveConnected && retry <= maxRetries) {
    ...
    interactiveReconnectTimeout = setTimeout(
      function(nextRetry) {
    ...
    renderWindow.webContents.send(
      "error",
      "Interactive reconnection failed several times. Stopping reconnection attempts."
    );
    logger.error(
      "Interactive reconnection failed several times. Stopping reconnection attempts."
    );
...
function constellationReconnect() {
  ...
  if (!constellationConnected && retry <= maxRetries) {
    ...
    constellationReconnectTimeout = setTimeout(
      function(nextRetry) {
    ...
    renderWindow.webContents.send(
      "error",
      "Constellation reconnection failed several times. Stopping reconnection attempts."
    );
    logger.error(
      "Constellation reconnection failed several times. Stopping reconnection attempts."
    );
