...
function addControlHandlers(controls) {
    const addHandler = (control, event) => {
        control.on(event, (inputEvent, participant) => {
            logger.debug("Control event: " + event + " for cotrol: ", inputEvent.input.controlID);
    ...
    try {
        ...
        if (gameName != null && retry <= 3) {
            try {
                ...
                let dbControls = profileManager.getJsonDbInProfile("/user-settings/controls/" + gameName);
