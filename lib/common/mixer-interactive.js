...
function refreshInteractiveCache(retry) {
    ...
    // Get last board name.
    let dbSettings = dataAccess.getJsonDbInProfile("/settings");

    try {
        let gameName = dbSettings.getData('/interactive/lastBoardId');

        if (!dataAccess.profileDataPathExistsSync(`/controls/${gameName}.json`)) {
            return;
        }
        ...
        if (gameName != null && retry <= 3) {
            try {
                ...
                let dbControls = dataAccess.getJsonDbInProfile("/controls/" + gameName);
