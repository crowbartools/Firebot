...
function updateButtonProperties(controlId, properties, trigger) {
    if (getInteractiveStatus()) {
        returnButtonById(controlId).then(async (control) => {
            ...
            if (properties.cooldown.shouldUpdate) {
                updateObj.cooldown = await getEvaluatedPropertyValue(control, 'cooldown', properties.cooldown.value, trigger);
                cooldowns.updateCooldownForControlId(controlId, updateObj.cooldown);
            }
