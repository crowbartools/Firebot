            ...
            mixerClient.synchronizeScenes()
                ...
                .then((res) => {
                    ...
                    scenes.forEach((scene) => {
                        ...
                        scene = mixerClient.state.getScene(scene.sceneID);
                        ...
                        // Bind mousedown event to each control.
                            ...
                            // Bind keydown event to each control.
                            control.on('keydown', (inputEvent, participant) => {
                            // Send control on to the control router.
                                Controls.router('mousedown', controls, control, interactiveCache, inputEvent, participant);
                            });
                            // Bind keyup event to each control.
                            control.on('keyup', (inputEvent, participant) => {
                            // Send control on to the control router.
                                Controls.router('mouseup', controls, control, interactiveCache, inputEvent, participant);
                            });
                            control.on('move', (inputEvent) => {
                            // Send control on to the control router.
...
function returnButtonById(buttonID) {
    return new Promise((resolve) => {
        let control = mixerClient.state.getControl(buttonID);
        resolve(control);
    });
}
...
function mixerConnect() {
    ...
    try {
        ...
        let versionid = dbControls.getData('./versionid');
    ...
    if (gameName !== null && gameName !== undefined) {
        if (retry <= 3) {
            try {
                // Get settings for last board.
                let dbControls = dataAccess.getJsonDbInUserData("/user-settings/controls/" + gameName);
                let controlJson = dbControls.getData('/');
        ...
        // We don't have a last used board to load.
        return;
    }
}
...
function getScenes() {
    ...
    let sceneNames = Object.keys(scenes).map((key) => {
...
function getGroups() {
    let defaultGroups = ['Pro', 'Subscribers', 'Moderators', 'Staff'];
...
function returnButton(buttonID, sceneID) {
    return new Promise((resolve) => {
        let scene = mixerClient.state.getScene(sceneID);
        let control = scene.getControl(buttonID);
        resolve(control);
    });
}
...
function mixerDisconnect() {
    ...
    //clear cooldowns
    // TODO: we need a new system for handling cooldowns
    //cooldownSaved = [];
