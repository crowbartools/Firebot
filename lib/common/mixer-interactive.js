...
function addControlHandlers(controls) {
  const addHandler = (control, event) => {
    control.on(event, (inputEvent, participant) => {
      logger.debug(
        "Control event: " + event + " for cotrol: ",
...
function refreshInteractiveCache(retry) {
  ...
  // Get last board name.
  let dbSettings = profileManager.getJsonDbInProfile("/user-settings/settings");
    ...
    if (gameName != null && retry <= 3) {
      try {
        ...
        let dbControls = profileManager.getJsonDbInProfile(
          "/user-settings/controls/" + gameName
        );
  ...
  return new Promise((resolve, reject) => {
    try {
      mixerClient
        ...
        .catch(err => {
          logger.error("Error syncin scenes", err);
          reject(err);
        });
    } catch (err) {
  ...
  if (groupid !== "None") {
    mixerClient.synchronizeGroups().then(() => {
      ...
      if (group !== undefined) {
        ...
        renderWindow.webContents.send(
          "error",
          "A button tried to change someone to a group:" +
            groupid +
            ". But, I couldnt get that group info from Mixer. Make sure it has a default scene set."
        );
      }
    });
  }
...
function mixerConnect() {
  let dbAuth = profileManager.getJsonDbInProfile("/user-settings/auth");

  // Get current controls board and set vars.
  try {
    // Get last board name.
    let dbSettings = profileManager.getJsonDbInProfile(
    ...
    mixerClient
      ...
      .then(() => {
        mixerClient.on("error", err => {
          logger.error("recieced error from mixer client:", err);
        ...
        makeAllScenes()
          .catch(err => {
            renderWindow.webContents.send(
              "error",
              "Error creating scenes on Mixer."
            );
          ...
          .catch(err => {
            renderWindow.webContents.send(
              "error",
              "Error creating the banned usergroup scene."
            );
          ...
          .catch(err => {
            renderWindow.webContents.send(
              "error",
              "Error creating groups on Mixer."
            );
          ...
          .catch(err => {
            renderWindow.webContents.send(
              "error",
              "Error with startup modification loop."
            );
        ...
        logger.info(
          `Connected Interactive server: ${mixerClient.socket.socket.url}`
        );
      ...
      .catch(err => {
        renderWindow.webContents.send(
          "error",
          "Error opening connection to Mixer. The service may be down, or there may be an issue with your auth tokens. Try re-logging into the app."
        );
    ...
    renderWindow.webContents.send(
      "error",
      "You need to import an interactive board before trying to connect."
    );
...
function getGroups() {
  ...
  let customGroups = [];
  let dbGroup = profileManager.getJsonDbInProfile("/user-settings/groups");
  try {
...
function updateButtonProperties(controlId, properties, trigger) {
  if (getInteractiveStatus()) {
    returnButtonById(controlId).then(async control => {
      // Send control off to Mixer to update.
      let updateObj = {};
      if (properties.cost.shouldUpdate) {
        updateObj.cost = await getEvaluatedPropertyValue(
          control,
          "cost",
      ...
      if (properties.text.shouldUpdate) {
        updateObj.text = await getEvaluatedPropertyValue(
          control,
          "text",
      ...
      if (properties.tooltip.shouldUpdate) {
        updateObj.tooltip = await getEvaluatedPropertyValue(
          control,
          "tooltip",
      ...
      if (properties.progress.shouldUpdate) {
        updateObj.progress = await getEvaluatedPropertyValue(
          control,
          "progress",
      ...
      if (properties.disabled.shouldUpdate) {
        updateObj.disabled =
          properties.disabled.value === "toggle"
            ? !control.disabled
