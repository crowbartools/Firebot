...
function changeGroups(participantInfo, groupidInfo) {
    ...
    if (groupid !== "None") {
        ...
        logger.log(`Attempting to move ${participant.username} to group ${groupid}...`);
            ...
            if (group != null && group.groupID) {
                ...
                mixerClient.updateParticipants({participants: [participant]})
                    .then(() => {
                        ...
                        logger.log("Successfully updated participant the first time...");

                        mixerClient.updateParticipants({participants: [participant]}).then(() => {
                            logger.log("Successfully updated participant the second time...");
                        });
