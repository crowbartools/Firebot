...
function createChatSocket (chatter, userId, channelId, endpoints, authkey) {
    return new Promise((resolve, reject) => {
        if (authkey != null) {
                ...
                socket.auth(channelId, userId, authkey)
                    .then((res) => {
                        ...
                        if (res.authenticated === true) {
                            ...
                            console.log(chatter + ' did not authenticate successfully.');
                            reject(res);
                        }
                    })
                    .catch(err => {
            ...
            console.log('No auth key provided to connect to chat for ' + chatter);
        }
    });
}
...
function getCurrentViewers() {
    console.log('Attempting to get current viewer count');
    ...
    return streamerClient.request('GET', 'channels/' + streamer.channelId, {
        ...
        .then(res => {
            ...
            console.log('Error getting current viewers.');
            console.log(err);
            return 0;
        });
}
...
exports.getGeneralChannelData = function(channelNameOrId, getBroadcastData = true) {
    return new Promise(async resolve => {
        ...
        try {
            ...
            console.log('Error getting general details.');
            console.log(err);
        ...
        if (getBroadcastData && generalDeets.online === true) {
            try {
                ...
                console.log('Error getting broadcast details.');
                console.log(err);
            }
        }
