...
function createClip(duration) {
    return new Promise(resolve => {
        logger.info("Attempted to create clip with length " + duration);
        ...
        logger.debug(`Creating clip with access token ${streamerData.accessToken} with duration of ${duration}s`);

        request.post('https://mixer.com/api/v1/clips/create', {
            'auth': {
                'bearer': streamerData.accessToken
            },
            'body': {
