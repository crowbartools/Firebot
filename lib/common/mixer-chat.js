...
function createChatSocket(chatter, userId, channelId, endpoints, authkey) {
    ...
    // No auth key was provided
    // Log error, and exit the function
    if (authkey == null) {
        logger.info("No auth key provided to connect to chat for " + chatter);

    // Unknown chatter type
    } else if (chatter !== "Streamer" && chatter !== "Bot") {
        return Promise.reject("Error creating chat socket for " + chatter + ".");
    ...
    // Checks passed
        ...
        // Return a new promise once authenticated to mixer's chat
...
function whisper(chatter, username, message) {
    ...
    if (chatter === "streamer") {
        try {
            ...
            logger.error("error sending whisper as streamer", err);
