...
const commandManager = require("../chat/command-manager");
...
function createChatDataProcessing(chatter) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("UserJoin", data => {
      ...
      let event = new LiveEvent(
        EventType.USER_JOINED_CHAT,
        EventSourceType.CHAT,
        { username: data.username, data: data }
      );
    ...
    socket.on("PollStart", data => {
      ...
      renderWindow.webContents.send("chatUpdate", data);
      let event = new LiveEvent(EventType.POLL_STARTED, EventSourceType.CHAT, {
    ...
    socket.on("PollEnd", data => {
      ...
      renderWindow.webContents.send("chatUpdate", data);
      let event = new LiveEvent(EventType.POLL_ENDED, EventSourceType.CHAT, {
        username: data.author.user_name,
        data: data
      });
      eventRouter.uncachedEvent(event);
    ...
    socket.on("DeleteMessage", data => {
      ...
      let event = new LiveEvent(
        ...
        EventSourceType.CHAT,
        { username: data.moderator.user_name, data: data }
      );
      eventRouter.uncachedEvent(event);
    });
    ...
    if (chatter === "Streamer") {
      ...
      socket.on("ChatMessage", data => {
        ...
        commandManager.handleChatEvent(data, "streamer").catch(reason => {
        ...
        let event = new LiveEvent(
          ...
          EventSourceType.CHAT,
          { username: data.user_name, data: data }
        );
        eventRouter.uncachedEvent(event);
      });
      ...
      socket.on("PollStart", data => {
        ...
        let event = new LiveEvent(
          ...
          EventSourceType.CHAT,
          { username: data.author.user_name, data: data }
        );
        eventRouter.cachedEvent(event, data.endsAt);
      });
      ...
      socket.on("PurgeMessage", data => {
        ...
        getUserInfo(data.user_id, user => {
          ...
          if (data.moderator !== undefined) {
            event = new LiveEvent(
              EventType.MESSAGE_PURGED,
              EventSourceType.CHAT,
              { username: user.username }
            );
            //otherwise it was the result of a ban
          } else {
            event = new LiveEvent(EventType.USER_BANNED, EventSourceType.CHAT, {
              username: user.username
            });
          }
      ...
      socket.on("ClearMessages", data => {
        ...
        let event = new LiveEvent(
          ...
          EventSourceType.CHAT,
          { username: data.clearer.user_name, data: data }
        );
        eventRouter.uncachedEvent(event);
      });
      ...
      // React to chat messages
      socket.on("ChatMessage", data => {
        commandManager.handleChatEvent(data, "bot");
...
function chatDisconnect() {
  if (global.streamerChat != null) {
    ...
    //flush cmd cooldowns
    commandManager.flushCooldownCache();
  }
...
function whisper(chatter, username, message) {
  ...
  if (chatter === "streamer") {
    try {
      ...
      renderWindow.webContents.send(
        "error",
        "There was an error sending a whisper to chat as the streamer. If you are testing a chat button, please make sure Interactive is connected."
      );
    }
  } else if (chatter === "bot") {
    try {
      ...
      renderWindow.webContents.send(
        "error",
        "There was an error sending a whisper to chat as the bot. If you are testing a chat button, please make sure Interactive is connected."
      );
    }
  }
}
...
function broadcast(chatter, message) {
  ...
  if (chatter === "streamer") {
    try {
      ...
      renderWindow.webContents.send(
        "error",
        "There was an error sending a message to chat as the streamer. If you are testing a chat button, please make sure Interactive is connected."
      );
    }
  } else if (chatter === "bot") {
    try {
      ...
      renderWindow.webContents.send(
        "error",
        "There was an error sending a message to chat as the bot. If you are testing a chat button, please make sure Interactive is connected."
      );
    }
  }
}
 ...
 */
function smartSend(message, username) {
  if (message === undefined) return;

  let chatter = accountAccess.getAccounts().bot.loggedIn ? "bot" : "streamer",
    shouldWhisper = username !== undefined && username !== "",
