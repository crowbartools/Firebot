...
function streamerConnect(streamer) {
    ...
    client.request('GET', `users/` + streamer['userId'])
        .then(response => {
...
function botConnect(botter) {
    ...
    // Request chat endpoints and connect.
    client.request('GET', `users/` + botter['userId'])
        .then(response => {
            userInfo = response.body;
            return new Mixer.ChatService(client).join(streamer['channelId']);
        })
        .then(response => {
            const body = response.body;
...
function getChatUserInfo(userID, callback) {
    ...
    client.request('GET', `chats/` + streamer['channelId'] + '/users/' + userID).then(response => {
        callback(response);
    })
        .catch(error => {
        // Log error for dev.
