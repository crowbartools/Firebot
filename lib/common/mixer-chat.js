        ...
        if (chatter === "Streamer") {
            ...
            // Send to command router to see if we need to act on a command.
                ...
                if (data.message && data.message.meta && data.message.meta["is_skill"]) {
                    ...
                    let skillEvent;
                    let stickerOnlyEvent;

                    // If embers, send as an embers event. Else send as sparks event.
                    if (skill.currency === "Embers") {
                        skillEvent = new LiveEvent(EventType.SKILL_EMBERS, EventSourceType.CHAT, {
                        ...
                        stickerOnlyEvent = new LiveEvent(EventType.SKILL_STICKER_EMBERS, EventSourceType.CHAT, {
                            username: data.user_name,
                            data: {
                                skill: skill
                            }
                        ...
                        skillEvent = new LiveEvent(EventType.SKILL, EventSourceType.CHAT, {
                            username: data.user_name,
                            data: {
                                skill: skill
                            }
                        ...
                        stickerOnlyEvent = new LiveEvent(EventType.SKILL_STICKER, EventSourceType.CHAT, {
                            username: data.user_name,
                            data: {
                                skill: skill
                            }
...
function botConnect(botter, channelId) {
    return new Promise((resolve) => {
        if (botter !== []) {
            let userInfo;
            ...
            botClient.request('GET', `users/` + botter['userId'])
                .then(response => {
                    userInfo = response.body;
                    return new Mixer.ChatService(botClient).join(channelId);
                })
                .catch(error => {
                    // Popup error.
                ...
                .then(response => {
                    const body = response.body;
                    return createChatSocket("Bot", userInfo.id, channelId, body.endpoints, body.authkey);
                })
                .catch(error => {
                    // Popup error.
...
function chatConnect() {
    return new Promise((resolve, reject) => {
        ...
        // Get Bot
        let botter = accountAccess.getAccounts().bot;
        if (botter == null) {
        ...
        streamerConnect(streamer)
            .then(() => {
                ...
                return botConnect(botter, streamer.channelId).then(() => {
                    resolve();
                });
            })
            .catch(err => {
