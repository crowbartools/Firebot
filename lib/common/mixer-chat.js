...
function createClip(title, duration) {
    return new Promise(async resolve => {
        ...
        if (!streamerData.authedForClips) {
            logger.warn("User not authed for clips");
            clipResult.reason = "You are not auth'd for clips. Please sign out and sign in again.";
        ...
        let createClipResponse = await streamerClient.request('POST', 'clips/create', { body: createClipRequest });
        ...
        if (createClipResponse.statusCode === 200) {
            ...
            clipResult.shareId = createClipResponse.body.shareableId;
        } else {
