...
function commandList(trigger) {
    ...
    // Pull the next command in the command array.
    // Add it's characters to the character count of command string. 
    // If we're not at 360, add the command to the command string.
    // If we're OVER 360 characters, send the current command sting, clear it, then added the new command to the new string.
    // Do this until we're out of commands in the command array.
