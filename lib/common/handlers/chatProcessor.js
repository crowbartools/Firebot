    ...
    // Get user specific settings
        ...
        if (populateReplaceVars) {
            ...
            try {
                ...
                message = await replaceVariableManager.evaluateText(message, trigger);
            } catch (err) {
...
function uiChatUserRefresh(page, chatUsers) {
    ...
    request(
        {
            url:
        "https://mixer.com/api/v1/chats/" +
        channelId +
        "/users?limit=100&page=" +
        page,
            auth: {
                bearer: accessToken
            }
        ...
        function(err, res) {
            ...
            user.username = user.userName;// eslint-disable-line
                ...
                chatUsers.push(user);
                userDb.setChatUserOnline(user);
            });

            // If this set of data is at max length, then run this function again on the next page.
            if (data.length === 100) {
                logger.info(
                    "Continuing to build chat userlist. On page " + page + " currently."
                );
                page = page + 1;
