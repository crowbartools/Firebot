...
function textProcessor(effect, trigger) {
    try {
        ...
        case "/mod":
        case "/unmod":
            renderWindow.webContents.send('error', "The chat command " + command + " is not yet supported.");
            break;
        case "/purge":
            renderWindow.webContents.send('error', "The purge command requires special permissions and Firebot doesn't ask for those. For now, this command is not supported.");
            break;
        default:
