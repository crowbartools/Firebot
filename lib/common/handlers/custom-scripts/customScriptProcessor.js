    ...
    // Make sure we first remove the cached version, incase there was any changes
        ...
        if (typeof customScript.run === "function") {
            setTimeout(function() {
                ...
                if (isV2Script) {
                  ...
                  param.value == null && param.value !== ""
                    ...
                    let accounts = {};
                    try {
                        accounts = profileManager.getJsonDbInProfile("/auth").getData("/");
              ...
              userCommand != null
                  ...
                  : null,
                    ...
                    response = getUserRoles(participant)
                        .then(roles => {
                ...
                */
                        ...
                        Object.values(EffectType).includes(type)
                                                ...
                                                renderWindow.webContents.send(
                                                    "error",
                                                    "Custom script tried to execute an unknown or unsupported effect type: " +
                            type
                                                );
                                            }
                                        ...
                                        if (
                                            responseObject.commands != null &&
                      responseObject.commands.length > 0
                                        ) {
                ...
                */
                                if (data) {
                                    if (data.success === true && data.message) {
                                        ...
                                        renderWindow.webContents.send(
                                            "error",
                                            "Custom script failed with the message: " + data.message
                                        );
                                    }
                                }
                            }
            ...
            renderWindow.webContents.send(
                ...
                "Error running '" +
          scriptName +
          "', script does not contain a visible run fuction."
            );
        }
    } catch (err) {
        renderWindow.webContents.send(
