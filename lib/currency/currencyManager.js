...
function applyCurrency() {
  ...
  logger.debug(`Got $${Object.values(currencyData).length} currencies`);
  ...
  Object.values(currencyData).forEach(currency => {
    ...
    logger.debug(
      `${currency.name} with interval ${
        currency.interval
      }, current mins: ${currentMinutes}, mod: ${intervalMod}`
    );
    if (intervalMod === 0) {
      // do payout
      logger.debug(
      ...
      logger.debug(`${currency.name} isnt ready to payout`);
    }
  });
}
