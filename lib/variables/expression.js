...
const $use = (handlers, handler) => {
    ...
    if (handler.triggers != null && !Array.isArray(handler.triggers)) {
        throw new TypeError('triggers must be null or an array');
    }
...
const $unuse = (handlers, handler) => {
    ...
    // validate: argsCheck argument
    } else if (typeof handler.rgsCheck !== 'function') {
...
class Expression {
    ...
    static useOld(handle, argsCheck, evaluator) {
