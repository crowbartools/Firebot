...
function mixerConnect(){
    ...
    try{
        ...
        mixerClient.open({
            ...
            .then((res) => { makeAllGroups(); }) // Make all needed groups.
            .then((res) => { Chat.connect(); }) // Connect to chat.
...
function makeScene(sceneName){
    ...
    try{
        ...
        // Push new controls to mixer.
        // If default we just need to make controls, otherwise we need to create the scene.
        if(sceneName == "default"){
            ...
            mixerClient.createScenes({scenes:[ { "sceneID": sceneName, "controls": makeControls(sceneName) } ] })
                .then(controls =>{
                    controls.forEach((control) => {
                        // Bind mousedown event to each control.
                        control.on('mousedown', (inputEvent, participant) => {
...
function changeScenes(group, scene){
    var sceneTest = mixerClient.state.getScene(scene);
    if(sceneTest !== undefined){
        ...
        console.log('This scene doesnt exist yet.');
    }    
}
