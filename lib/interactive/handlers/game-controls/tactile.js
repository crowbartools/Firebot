...
function buttonPresser(eventType, key, keyNum, oppositeKey, oppNum, holding){

    // See if this is a key that needs to be held down or not.
    if(holding == "Yes"){
        keyHolding(key, oppositeKey, keyNum);
    } else {
        if(eventType == "mousedown"){
            keyTap(key, oppositeKey);
        }
    }
...
function keyTap(key, oppositeKey){
    // Press Key
...
function keyHolding(key, oppositeKey, keyNum){
