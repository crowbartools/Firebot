...
function buttonRouter(eventType, effect, control){
    ...
    if(oppositeKey !== undefined){
        ...
        if(keyNum >= oppNum){
            // Press key
            tactile.buttonPress(eventType, key, keyNum, oppositeKey, oppNum, holding);
        } else {
            // Press Opposite Key
            tactile.buttonPress(eventType, oppositeKey, oppNum, key, keyNum, holding);
        }
    } else {
        // Press Key
        tactile.buttonPress(eventType, key, keyNum, false, oppNum, holding);
    }
}
