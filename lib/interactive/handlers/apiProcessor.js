	...
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
        ...
        try{
			...
			console.log('Error sending advice to chat.')
        }   
	  } else {
		  console.log('Error hitting the random advice api.')
	  }
	})
}
	...
	randomPuppy('catpictures')
		.then(url => {
			try{
				...
				console.log('Error sending cat picture to chat.')
			}	
	})	
}
	...
	randomPuppy('dogpictures')
		.then(url => {
			try{
				...
				console.log('Error sending dog picture to chat.')
			}	
	})		
}
	...
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
			...
			try{
				...
				console.log('Error sending cat fact to chat.')
			}		
	  } else {
		  console.log('Error hitting the cat fact api.')
	  }
	})
}
	...
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
			...
			try{
				...
				console.log('Error sending dog fact to chat.')
			}		
	  } else {
		  console.log('Error hitting the dog fact api.')
	  }
	})
}
	...
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
			...
			try{
				...
				console.log('Error sending pokemon to chat.')
			}		
	  } else {
		  console.log('Error hitting pokemon API.')
	  }
	})
}
	...
	randomPuppy('aww')
		.then(url => {
			try{
				...
				console.log('Error sending Aww to chat.')
			}	
	})				
}
	...
	request(url, function (error, response, body) {
	  if (!error && response.statusCode == 200) {
       ...
       chat.broadcast(chatter, 'Number Trivia: '+body);
		  ...
		  console.log('Error sending Number Trivia to chat.')
	  }
	})
}
