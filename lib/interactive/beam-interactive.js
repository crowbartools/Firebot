...
function makeScene(sceneID){
    ...
    // Push new controls to beam. (Scene id default until beam incorporates scenes into node.)
    beamClient.createControls({"sceneID": 'default', "controls": makeControls(sceneID)})
        .then(controls =>{
            controls.forEach((control) => {
                // Store etags for each button for later use.
                var controlId = control.controlID;
                var etag = control.etag;
                dbControls.push('./firebot/controls/'+controlId+'/etag', etag);

                // Bind mousedown event to each control.
                control.on('mousedown', (inputEvent, participant) => {
                    // Send control on to the control router.
                    Controls.router(gameJson, inputEvent, participant);
