...
function autoGroup(groupList, username, userid, participant) {
    ...
    // See if username matches a custom group, if it does put them in there and then end. Else keep going.
        for (let item of groupList.customGroups) {
            try {
                let dbGroups = profileManager.getJsonDbInProfile("/groups");
                ...
                for (let user of groupJson) {
                    if (username.toLowerCase() === user.toLowerCase()) {
                        ...
                        renderWindow.webContents.send("eventlog", {
                            type: "general",
                            username: username,
                            event: "was placed into the " + item + " group."
                        });
            ...
            Chat.getUser(userid, function(response) {
                ...
                if (mainGroup === "Staff" && groupList.staffGroup === true) {
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Staff group."
                    });
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Streamer group."
                    });
                } else if (
                    mainGroup === "ChannelEditor" &&
          groupList.editorGroup === true
                ) {
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Channel Editors group."
                    });
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Moderator group."
                    });
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Subscriber group."
                    });
                    ...
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "was placed into the Pro group."
                    });
                } else {
                    renderWindow.webContents.send("eventlog", {
                        type: "general",
                        username: username,
                        event: "will remain in the default group."
                    });
                }
    ...
    // We have people waiting! Check to see if we're still connected to both chat and interactive.
        ...
        if (connectionChecker()) {
            ...
            renderWindow.webContents.send("eventlog", {
                ...
                username: "System:",
                event:
          "Interactive auto grouping stopped. Please connect to both chat and interactive to restart."
            });
            groupQueueStop();
        }
    } else {
...
function groupChecker() {
    ...
    // Get last board name.
        ...
        let dbControls = profileManager.getJsonDbInProfile("/controls/" + gameName);
...
function groupQueueStart() {
    ...
    if (groupList !== false && connectionChecker()) {
        ...
        renderWindow.webContents.send("eventlog", {
            ...
            username: "System:",
            event:
        "User groups detected and we are fully connected. Starting auto grouping."
        });
        ...
        logger.info(
            "Not connected to both chat and interactive. Stalling auto grouping."
        );
        renderWindow.webContents.send("eventlog", {
            type: "general",
            username: "System:",
            event:
        "Please connect to both chat and interactive to automatically group participants for interactive."
        });
    } else {
        ...
        renderWindow.webContents.send("eventlog", {
            type: "general",
            username: "System:",
            event: "No user groups detected. Skipping auto grouping."
        });
    }
}
