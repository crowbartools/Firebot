...
function tactileProgress(tactile) {
    ...
    for (i = 0; i < tactile.length; i++) {
        ...
        if ( tactile[i].pressFrequency > 0 && cooldownGroup !== "solo" || tactile[i].holding > 0 && cooldownGroup !== "solo"){
            ...
            var progress = dbControls.getData('/cooldowns/'+cooldownGroup+'/progress');
            // Loop through the pre-built report in the controls file.
            for (a = 0; a < progress.length; a++) {
                var button = progress[a];
                var buttonid = progress[a].id;
                var buttonProgress = button;
            ...
            if(savedReport === undefined && didAnimate === true){
                // No report saved yet. Make a new one.
                var buttonProgress = {
                    "id": rawid,
                    "fired": false,
                    "progress": 0
