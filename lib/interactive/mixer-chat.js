...
const commandHandler = require("./handlers/chat-only/commandHandler.js");
...
function createChatSocket (chatter, userId, channelId, endpoints, authkey) {
    ...
    if(chatter == "Streamer"){
        ...
        global.streamerChat.on('ChatMessage', data => {
          ...
          if(commandHandler.messageIsCommand(message)) {
            commandHandler.handleChatCommand(data, "streamer");
          }
        });
        ...
        global.botChat.on('ChatMessage', data => {
          ...
          var message = data.message.message[0].data;
          if(commandHandler.messageIsCommand(message)) {
            commandHandler.handleChatCommand(data, "bot");
          }
