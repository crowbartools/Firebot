...
function permissionsRouter(participant, control) {
  return new Promise((resolve, reject) => {
    ...
    if (
      mixerInteractive.getInteractiveStatus() === true &&
      mixerChat.getChatStatus() === true &&
      control.permissionType != null
    ) {
      ...
      if (control.permissionType === "Group") {
        ...
        getUserRoles(participant)
          .then(userRoles => {
            ...
            if (control.permissions instanceof Array) {
              ...
              userHasPermission = userIsInRole(
                getCombinedRoles(participant.username, userRoles),
                mapRoleNames(control.permissions)
              );
            } else {
            ...
            if (!userHasPermission) {
              ...
              renderWindow.webContents.send("eventlog", {
                ...
                event:
                  "pressed the " +
                  control.controlId +
                  " button, but does not have permission."
              });
          ...
          .catch(err => {
            ...
            renderWindow.webContents.send(
              "error",
              "There was an error trying to get user roles from chat for button permissions." +
                err
            );
        ...
        if (
          ...
          mixerChat.whisper(
            "bot",
            participant.username,
            "You do not have permission to use the " + control.text + " button!"
          );
          renderWindow.webContents.send("eventlog", {
            type: "general",
            username: participant.username,
      ...
      logger.debug(
        "Either we are not connected to both chat and interactive, or this button has no permissions. Skipping permissions check."
      );
      resolve(
        "Either we are not connected or this button has no permissions. Skipping check."
      );
    }
  });
}
