...
function controlRouter(
  ...
  if (inputevent !== "mouseup" && inputevent !== "keyup") {
    ...
    permissions.router(participant, control).then(() => {
      logger.debug("Checking cooldowns");
      // Make sure cooldowns is processed.
      cooldowns
        .router(mixerControls, mixerControl, firebot, control)
        .then(() => {
            ...
            .then(() => {
              ...
              if (control.skipLog !== true) {
                renderWindow.webContents.send("eventlog", {
                  type: "general",
                  username: participant.username,
                  event: "pressed the " + controlID + " button."
                });
              }
              ...
              if (control.skipLog !== true) {
                renderWindow.webContents.send("eventlog", {
                  ...
                  event:
                    "tried to press " +
                    controlID +
                    " but it has not passed its threshold."
                });
              }
            })
            .then(() => {
              ...
              if (inputEvent.transactionID) {
                logger.debug(
                  "input event has transaction id, checking spark exempt status..."
                );
                // This will charge the user.
                sparkExemptManager.userIsExempt(participant).then(exempt => {
                  if (exempt === false) {
                    logger.debug(
                      participant.username +
                        " is not exempt, capturing transaction to charge sparks."
                    );
        ...
        .catch(() => {
          ...
          if (control.skipLog !== true) {
            renderWindow.webContents.send("eventlog", {
              type: "general",
              username: participant.username,
              event: "tried to press " + controlID + " but it is on cooldown."
            });
          }
        });
    });
