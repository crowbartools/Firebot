        ...
        cooldown.router(mixerControls, mixerControl, firebot, control, function(response){
            ...
            for (var item in effects){
                ...
                switch(effectType){
                    ...
                    case "Custom Script":
                        try {
                          renderWindow.webContents.send('error', "running script");
                          customScriptHandler.processScript(effect.scriptName,
                            control.controlId, username);
                        } catch(err) {
    ...
    for (var item in effects){
        ...
        switch(effectType){
            ...
            case "Custom Script":
                try {
                  customScriptHandler.processScript(effect.scriptName,
                    control.controlId, "Test Username");
                } catch(err) {
                  renderWindow.webContents.send('error', "Oops! There was an error processing the custom script.");
                }
                break;
