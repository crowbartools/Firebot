                        ...
                        threshold.router(control)
                            ...
                            .then(() => {
                                ...
                                if (inputEvent.transactionID) {
                                    if (sparkExemptManager.checkUseExemption() === true) {
                                        logger.info("input event has transaction id and exempt people, checking spark exempt status...");
                                        // This will charge the user.
                                        sparkExemptManager.userIsExempt(participant)
                                            .then((exempt) => {
                                                if (exempt === false) {
                                                    logger.info(participant.username + " is not exempt, capturing transaction to charge sparks.");
                                                    renderWindow.webContents.send('eventlog', {type: "general", username: 'System', event: participant.username + " is not exempt, capturing transaction to charge sparks."});
                                                    mixerInteractive.sparkTransaction(inputEvent.transactionID);
                                                }
                                            });
                                    } else {
                                        logger.info("No exempt groups or users to check against, charge the sparks!");
                                        renderWindow.webContents.send('eventlog', {type: "general", username: 'System', event: "No exempt users or groups set, so we are charging sparks!"});
                                        mixerInteractive.sparkTransaction(inputEvent.transactionID);
                                    }
                                }
                            });
