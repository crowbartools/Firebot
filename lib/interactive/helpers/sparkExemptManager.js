  ...
  return new Promise(resolve => {
    ...
    if (exemptUsers.length > 0) {
      if (exemptUsers.includes(userName)) {
        ...
        logger.info(
          `${userName} is on exempt list. Not charging sparks. Exempt users: ${exemptUsers.join()}`
        );
        resolve(true);
      }
    }
    ...
    if (exemptGroups.includes("default")) {
      logger.info(
        "Default group is selected as spark exempt. Free buttons for everyone!"
      );
      resolve(true);
    }
    ...
    permissions
      ...
      .then(userRoles => {
        let combinedRoles = permissions.getCombinedRoles(
        ...
        if (permissions.userIsInRole(combinedRoles, exemptGroupMap)) {
          ...
          logger.info(
            `${userName} is in an exempt group or is the owner. Not charging sparks. Exempt groups: ${exemptGroupMap}, user groups: ${combinedRoles}`
          );
          resolve(true);
        } else {
          resolve(false);
      ...
      .catch(err => {
        ...
        renderWindow.webContents.send(
          "error",
          "There was an error trying to get user roles from chat for spark exemptions." +
            err
        );
        resolve(false);
      });
