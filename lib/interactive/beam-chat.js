...
var dbAppSettings = new JsonDB('./app-settings/settings', true, false);
...
function chatConnect(){
    ...
    try{
        ...
        console.log('You need to to log into the app as a streamer. (chat-connect)');
        return;
    }
...
function streamerConnect(streamer){
    ...
    // Bot Login
    beam.use('oauth', {
        clientId: dbAppSettings.getData('/client_Id'),
        tokens: {
    ...
    .catch(error => {
        // Popup error.
        console.log('Couldnt connect to chat as streamer. (chat-connect)');
...
function createBotChatSocket (chatter, userId, channelId, endpoints, authkey) {
    ...
    if(chatter == "Streamer"){
        ...
        global.streamerChat.on('error', error => {
            // Popup error.
            console.log('There was an error with the chat socket for '+chatter+'. Trying to reconnect in 5 seconds. (chat-connect)')
...
// Chat Reconnect 
// This tries to reconnect to chat.
...
function whisper(chatter, username, message){
     if (chatter == "Streamer"){
        try{
            ...
            console.log('There was an error sending your message from '+chatter+'. (chat-connect)');
        }
     } else if (chatter == "Bot") {
        try{
