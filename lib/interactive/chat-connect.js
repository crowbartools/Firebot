...
function streamerConnect(){
    ...
    try{
        ...
        errorLog.log('You need to log into the app as a streamer. (chat-connect)');
...
function createBotChatSocket (chatter, userId, channelId, endpoints, authkey) {
    ...
    if(chatter == "Streamer"){
        ...
        global.streamerChat.on('error', error => {
            // Popup error.
            errorLog.log('There was an error with the chat socket for '+chatter+'. Trying to reconnect in 5 seconds. (chat-connect)')
            // Send error message to gui
            renderWindow.webContents.send('chat-disconnect');
