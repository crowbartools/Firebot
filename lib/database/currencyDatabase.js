...
function adjustCurrency(user, currencyId, value) {
    return new Promise(resolve => {
        ...
        logger.debug(
            "Adjusting " + value + " currency to " + user.username + ". " + currencyId
        );
...
function adjustCurrencyForUser(username, currencyId, value) {
    return new Promise((resolve, reject) => {
        ...
        // Validate inputs.
        if (username === null || currencyId === null || value === null || value === NaN) {
            return resolve(false);
        }
...
function addCurrencyToUserGroupOnlineUsers(roles = [], currencyId, value) {
    return new Promise(resolve => {
        ...
        if (roles === [] || currencyId === null || value === null || value === 0) {
            return resolve();
        }
...
function addCurrencyToOnlineUsers(currencyId, value) {
    return new Promise((resolve, reject) => {
        ...
        // Don't do anything for 0 points.
        if(value === 0){
            return resolve();
        }
