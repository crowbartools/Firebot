...
function setChatUserOffline(user) {
    return new Promise((resolve, reject) => {
        if (!isViewerDBOn()) {
            return resolve();
        }
        ...
        // On occasion user is given to us as just the userid int and no additional info.
        // So, check to see if they give us the full object or just the id.
        ...
        // Update the record.
        db.update({ _id: userid }, { $set: updateDoc }, {}, function(err) {
            if (err) {
                logger.error("Error adding currency to user.", err);
            } else {
                logger.debug("Set user " + userid + " to offline.");
            }
            return resolve();
        });
    });
...
function setChatUserOfflineById(id) {
    return new Promise(resolve => {
        db.find({ _id: id }, (err, user) => {
            resolve(setChatUserOffline(user));
        });
    });
}
