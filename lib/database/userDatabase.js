        ...
        db.find({ _id: id }, (err, user) => {
            ...
            db.update({ _id: user._id }, { $set: updateDoc }, {}, function(err) {
                if (err) {
                    ...
                    logger.debug("Set " + user[0].username + "(" + user[0]._id + ") to offline.");
                }
                return resolve();
            });
        }); // End find
...
function setAllUsersOffline() {
    return new Promise(resolve => {
        ...
        db.find({ online: true }, (err, docs) => {
            let promises = docs.map(user => setChatUserOffline(user._id));
...
function connectUserDatabase() {
    logger.info('Trying to connect to user database...');
    if (!isViewerDBOn()) {
        return;
    }
    ...
    db.loadDatabase(err => {
        if (err) {
            logger.info("Error Loading Database: ", err.message);
            logger.info("Failed Database Path: ", path);
        }
    });

    logger.info("User Database Loaded: ", path);
    setAllUsersOffline();
    setLastSeenDateTime();
