...
// Helpers
// These help manage the database. Need to load them here to begin catching events from renderer.
// So this might look like it's not used, but it is...
...
function createUserFromChat(data) {
  return new Promise((resolve, reject) => {
    ...
    ins = currencyDatabase.addCurrenciesToNewUser(ins);
...
// This allows us to pass a field name, or a dot notation field name.
function updateField(obj, is, value) {
  if (typeof is == "string") return updateField(obj, is.split("."), value);
  else if (is.length === 1 && value !== undefined) return (obj[is[0]] = value);
  else if (is.length === 0) return obj;
  return updateField(obj[is[0]], is.slice(1), value);
}
...
function updateDbCell(changePacket) {
  ...
  return new Promise((resolve, reject) => {
    ...
    getUserById(id).then(user => {
      updateField(user, field, newValue);
      db.update({ _id: id }, user, err => {
        if (err) {
          reject(err);
        }
