...
class ImportedSetupManager extends JsonDbManager {
    constructor() {
        super("Imported Setup", "/imported-setups");
    }

    findAndReplaceCurrency(data, currency) {
    ...
    replaceQuestionAnswers(data, questions) {
        const entries = Object.entries(data);
    ...
    replaceCurrency(components, currency) {
        // loop through every component type (command, event, etc)
        for (const componentArray of Object.values(components)) {
            // loop through each component
            for (const component of componentArray) {
                this.findAndReplaceCurrency(component, currency);
            }
        }
    }

    async importSetup(setup, selectedCurrency) {
        ...
        if (quickActions.length > 0) {
            for (const action of quickActions) {
                ...
                importedSetup.components.quickActions.push(action.id);
            }
    ...
    removeSetupComponents(components) {
...
const importedSetupManager = new ImportedSetupManager();

frontendCommunicator.onAsync("import-setup", async ({setup, selectedCurrency}) => {
    return importedSetupManager.importSetup(setup, selectedCurrency);
});

frontendCommunicator.onAsync("remove-setup-components", async ({components}) => {
    return importedSetupManager.removeSetupComponents(components);
});

module.exports = importedSetupManager;