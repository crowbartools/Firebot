...
const serviceValidators = {
  ...
  integration: async (integrationId: string, validateFor: ValidationMode) => {
    if (
      validateFor === "execution" &&
      integrationManager.integrationCanConnect(integrationId)
    ) {
...
export const checkEffectDependencies = (
  ...
  if (dependencies.integrations) {
    ...
    return Object.entries(integrationDependencies).every(
      ([integrationId, isDependency]) => {
        return (
          !isDependency ||
          serviceValidators.integration(integrationId, validateFor)
        );
      }
    );
  }
