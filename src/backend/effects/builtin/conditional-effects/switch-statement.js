...
const model = {
    definition: {
        ...
        description: "Conditionally run effects based on a value",
    ...
    optionsTemplate: `
        ...
        <div ui-sortable="sortableOptions" ng-model="effect.cases">
            <div ng-repeat="caseStatement in effect.cases" style="margin-bottom: 15px;">
                <condition-section header="{{$index === 0 ? 'If' : 'Else If'}}" label="caseStatement.label" initially-open="$index === 0 && openFirst">
                    <condition-list condition-data="caseStatement.conditionData" trigger="trigger" trigger-meta="triggerMeta"></condition-list>
                    <div style="font-size: 15px;font-family: 'Quicksand'; color: #c0c1c2;margin-bottom:3px;">Then run the following effects:</div>
                    <effect-list effects="caseStatement.effectData"
                        trigger="{{trigger}}"
