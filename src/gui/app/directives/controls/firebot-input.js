            ...
            template: `
                <div style="{{$ctrl.style}}" class="{{$ctrl.class}}">
                    <div ng-if="$ctrl.useInputGroup" class="input-group">
                        ...
                        <input ng-if="!$ctrl.useTextArea" type="{{$ctrl.disableVariables ? $ctrl.inputType || 'text' : 'text'}}" class="form-control" ng-model="$ctrl.model" ng-change="$ctrl.onChange($ctrl.model)" placeholder="{{$ctrl.placeholderText}}"  replace-variables="{{$ctrl.dataType}}" disable-variable-menu="$ctrl.disableVariables" menu-position="{{$ctrl.menuPosition}}" button-position="{{$ctrl.varBtnPosition}}">
                        <textarea ng-if="$ctrl.useTextArea" ng-model="$ctrl.model" ng-change="$ctrl.onChange($ctrl.model)" class="form-control" placeholder="{{$ctrl.placeholderText}}" rows="4" cols="40"  replace-variables="{{$ctrl.dataType}}" disable-variable-menu="$ctrl.disableVariables" menu-position="{{$ctrl.menuPosition}}" button-position="{{$ctrl.varBtnPosition}}"></textarea>
                    </div>
