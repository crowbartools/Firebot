...
function keyTap(key, modifiers) {
    ...
    // Press as KBMRobot
        try {
            logger.info(
                "KBMRobot: Tapped " + key + " with modifiers " + cleanModifiers
            );
            // Special check for mouse clicks
            /*if (isMouseClick === true) {
                kbmRobot.mouseClick(key).go();
    ...
    // Press as robotjs.
        try {
            logger.info(
                "Robotjs: Tapped " + key + " with modifiers " + cleanModifiers
            );
            // Special check for mouse clicks
            if (isMouseClick === true) {
...
function keyHolding(key, state, modifiers) {
    ...
    if (emulator === "KBMRobot") {
        if (state === "down") {
            try {
                logger.info(
                    "KBMRobot: Held " + key + " and modifiers " + cleanModifiers
                );
                // Special check for mouse clicks
                if (isMouseClick === true) {
                    kbmRobot.mousePress(key).go();
                } else if (cleanModifiers.length === 0) {
            ...
            try {
                logger.info(
                    "KBMRobot: Released " + key + " and modifiers " + cleanModifiers
                );
                // Special check for mouse clicks
                if (isMouseClick === true) {
                    kbmRobot.mouseRelease(key).go();
                } else if (cleanModifiers.length === 0) {
