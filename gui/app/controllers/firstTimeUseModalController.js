...
(function() {
  ...
  angular
    .module("firebotApp")
    .controller("firstTimeUseModalController", function(
      logger,
      $rootScope,
      $scope,
      $uibModalInstance,
      ...
      function validateUserSettingsFolder(filePath) {
        ...
        if (!fs.existsSync(filePath) || !filePath.endsWith("user-settings")) {
          ...
          $scope.importErrorMessage =
            "This is not a valid 'user-settings' folder.";
        } else {
      ...
      function validateBackupZip(filepath) {
        ...
        return fs
          ...
          .pipe(
            unzipper
              .Parse() //eslint-disable-line new-cap
              .on("entry", entry => {
      ...
      $scope.openBackupBrowser = function() {
        ...
        listenerService.registerListener(registerRequest, filepath => {
          validateBackupZip(filepath).then(
            resp => {
              if (resp.valid) {
                ...
                $scope.importErrorOccured = true;
                $scope.importErrorMessage =
                  "Provided backup zip does not appear to be valid!";
              }
            },
            e => {
      ...
      $scope.addBoard = function() {
        ...
        boardService.loadBoardWithId(boardId).then(
          boards => {
            let board = boards[0];
