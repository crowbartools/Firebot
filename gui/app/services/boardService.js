       ...
       */
                        ...
                        try {
                            ...
                            if (exists) {
                                ...
                                renderWindow.webContents.send(
                                    "error",
                                    "Well this is weird. The board you tried to delete is already gone. Try restarting the app."
                                );
                                resolve();
                            }
                        } catch (err) {
            ...
            function backendCleanup(dbControls) {
                return new Promise(resolve => {
                    logger.info(
                        "Backend Cleanup: Checking for differences between mixer and firebot boards."
                    );
                    ...
                    try {
                        ...
                        logger.info(
                            "It doesnt appear that we have previously saved settings, skipping board cleanup"
                        );
                        resolve(true);
                        return;
                    }
                    ...
                    if (firebotSettings.joysticks) {
                        ...
                        let deletedJoysticks = firebotJoysticks.filter(
                            id => !mixerJoysticks.includes(id)
                        );
                        ...
                        deletedJoysticks.forEach(joystickId => {
                            try {
                                ...
                                logger.info(
                                    "Joystick " +
                    joystickId +
                    " is not on the mixer board. Deleting."
                                );
                            } catch (err) {
                                logger.error(err);
                            }
                    ...
                    firebotButtonArray = firebotButtonArray.filter(
                        val => !mixerButtonArray.includes(val)
                    );

                    // Filter out all scenes that match. Anything left in the firebotScenenArray no longer exists on the mixer board.
                    firebotSceneArray = firebotSceneArray.filter(
                        val => !mixerSceneArray.includes(val)
                    );
                        ...
                        try {
                            if (button === "") {
                                utilityService.showErrorModal(
                                    "Detected a button with a name that shouldnt be possible and could cause issues. Please reach out to the Firebot Dev team for help (Click the About link in the sidebar to find our Discord)."
                                );
                                continue;
                            }
                            dbControls.delete("./firebot/controls/" + button);
                            logger.info(
                                "Button " + button + " is not on the mixer board. Deleting."
                            );
                            ...
                            for (let cooldown in firebotSettings.cooldownGroups) {
                                if (firebotSettings.cooldownGroups.hasOwnProperty(cooldown)) {
                                    let cooldownButtons = dbControls.getData(
                                        "./firebot/cooldownGroups/" + cooldown + "/buttons"
                                    );
                    ...
                    for (let scene of firebotSceneArray) {
                        try {
                            ...
                            logger.info(
                                "Scene " +
                  scene +
                  " is not on the mixer board. Deleting from firebot."
                            );
                        } catch (err) {
                            logger.error(err);
                        }
            ...
            function backendBuilder(
                ...
                logger.info(
                    "Backend builder is pushing settings to " +
            gameName +
            " (" +
            versionid +
            ")."
                ...
                settingsService.setBoardLastUpdatedDatetimeById(
                    versionIdInfo,
                    gameName,
                    gameUpdated
                );
                ...
                if (
                    ...
                    if (
                        ...
                        try {
                            ...
                            logger.info(
                                "Converted control file " +
                  gameName +
                  ".json to version id format."
                            );
                        } catch (err) {
                            logger.error(err);
                            utilityService.showErrorModal(
                                "Unable to convert controls file " +
                  gameName +
                  ".json to new format. Do you have the file open somewhere? If so, close it down and restart Firebot."
                            );
                            return;
                        }
                    } else {
                        logger.info(
                            "We detected a control file still using the board name, but it looks like it has already been converted so we will ignore it."
                        );
                    }
                }
                ...
                return backendCleanup(dbControls).then(() => {
                    let controlID,
                        sceneName,
                        sceneControls,
                        emojiTest,
                    ...
                    for (let i = 0; i < gameJson.length; i++) {
                        ...
                        for (let a = 0; a < sceneControls.length; a++) {
                            ...
                            if (button.controlID.includes("/")) {
                                utilityService.showErrorModal(
                                    `A control with the id of '${
                                        button.controlID
                                    }' contains a forwardslash which can cause major issues. Please rename the control to remove the / from the name. Sorry for the inconvienence.`
                                );
                                continue;
                            }
                                    ...
                                    try {
                                        ...
                                        if (emojiTest === false) {
                                            ...
                                            utilityService.showErrorModal(
                                                "Button: " +
                          button.controlID +
                          " has emoji in the button name. This will cause all buttons to become unresponsive on connecting. Please remove emoji from the button name field in the Mixer studio. Note that button text is what is visible to viewers, and it's fine to have emoji there."
                                            );
                                        }
                                    ...
                                    /*
                                        ...
                                        */
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/controlId",
                                        controlID
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/scene",
                                        sceneName
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/text",
                                        text
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/cost",
                                        cost
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/kind",
                                        type
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/meta",
                                        button.meta
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/tooltip",
                                        button.tooltip
                                    );
                                }
                                ...
                                if (type === "textbox") {
                                    ...
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/multiline",
                                        button.multiline
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/hasSubmit",
                                        button.hasSubmit
                                    );
                                    dbControls.push(
                                        "./firebot/controls/" + controlID + "/placeholder",
                                        button.placeholder
                                    );
                                }
                                ...
                                if (type === "joystick") {
                                    ...
                                    dbControls.push(
                                        `./firebot/joysticks/${joystick.controlID}`,
                                        joystick
                                    );
                                }
                            } catch (err) {
                                logger.error(
                                    "Problem getting button info to save to json.",
                                    err
                                );
                            }
                        }
                        ...
                        if (sceneName.includes("/")) {
                            utilityService.showErrorModal(
                                "Scene: '" +
                  sceneName +
                  "' contains a forward slash (/) in it's name which can cause errors when connecting to Interactive. It is recommended that you rename your scene to use an & or + instead and resync with Firebot."
                            );
                        }
                        try {
                            ...
                            dbControls.push(
                                "./firebot/scenes/" + sceneName + "/sceneName",
                                sceneName
                            );
                            if (sceneName !== "default") {
                                ...
                                dbControls.push(
                                    "./firebot/scenes/" + sceneName + "/default",
                                    []
                                );
                            }
                        }
            ...
            function loadBoardById(id, forceSync = false) {
                ...
                return $http
                    .get("https://mixer.com/api/v1/interactive/versions/" + id)
                    .then(
                        function(response) {
                            ...
                            if (data.controlVersion === "1.0") {
                                utilityService.showErrorModal(
                                    "The board you're trying to load was created using Mixer Interactive v1. Please create a new board using Mixer Interactive v2."
                                );
                                return false;
                            }
                            ...
                            if (data.id !== null && data.game == null) {
                                ...
                                utilityService.showErrorModal(
                                    `The board (${id}) has been deleted on the Mixer Dev Lab. It has been automatically removed from Firebot.`
                                );
                                return false;
                            }
                            ...
                            try {
                                ...
                                try {
                                    // Checking if the data for this board is present in settings.json
                                    boardUpdated = settingsService.getBoardLastUpdatedDatetimeById(
                                        id
                                    );
                                    ...
                                    if (boardUpdated != null) {
                                        ...
                                        if (!boardExists) {
                                            logger.info(
                                                "Board was in settings, but the controls file is missing. Rebuilding."
                                            );
                    ...
                    loadedIds => {
                        ...
                        _.each(loadedIds, function(id) {
                            ...
                            try {
                                ...
                                logger.error(
                                    "Board " + id + " errored out while trying to load." + err
                                );
       ...
       */
            ...
            service.loadAllBoards = function() {
                ...
                if (knownBoards !== null && knownBoards !== undefined) {
                    ...
                    logger.debug(
                        "Loading boards: " + boardVersionIds
                            ? boardVersionIds.join(", ")
                            : "No boards found"
                    );
                    /* Step 2 */
                    // Load each board.
            ...
            service.saveCooldownGroupForCurrentBoard = function(
                previousName,
                cooldownGroup
            ) {
                if (previousName != null && previousName !== "") {
                    service.deleteCooldownGroupForCurrentBoard(
                        previousName,
                        cooldownGroup
                    );
                }
                ...
                if (cleanedCooldownGroup.buttons != null) {
                    cleanedCooldownGroup.buttons.forEach(buttonName => {
                        boardDb.push(
                            `./firebot/controls/${buttonName}/cooldownGroup`,
                ...
                boardDb.push(
                    "./firebot/cooldownGroups/" + cooldownGroup.groupName,
                    cleanedCooldownGroup
                );
            ...
            service.deleteCooldownGroupForCurrentBoard = function(
                cooldownGroupName,
                cooldownGroup
            ) {
                let boardDb = profileManager.getJsonDbInProfile(
