        ...
        .factory('boardService', function (logger, $http, $q, settingsService, $rootScope, utilityService) {
            ...
            */
            ...
            function deleteBoard(boardId) {
                return $q.when(new Promise((resolve) => {
                    ...
                    try {
                        let filepath = profileManager.getPathInProfile('/user-settings/controls/' + boardId + '.json');
            ...
            function backendBuilder(gameNameId, gameJsonInfo, gameUpdatedInfo, versionIdInfo, utilityService) {
                ...
                if (profileManager.profileDataPathExistsSync('/user-settings/controls/' + gameName + '.json')) {
                    if (!profileManager.profileDataPathExistsSync('/user-settings/controls/' + versionid + '.json')) {
                        logger.info('Converting control files to new versionid format.');
                        let oldPath = profileManager.getPathInProfile("/user-settings/controls/" + gameName + '.json');
                        let newPath = profileManager.getPathInProfile("/user-settings/controls/" + versionid + '.json');
                ...
                let dbControls = profileManager.getJsonDbInProfile("/user-settings/controls/" + versionid);
                        ...
                        try {
                            ...
                            try { // Checking if the data for this board is present in settings.json
                                ...
                                if (boardUpdated != null) {
                                    let boardExists = profileManager.profileDataPathExistsSync("/user-settings/controls/" + id + ".json");
                                    if (!boardExists) {
            ...
            function loadBoardsById(boardVersionIds, clearPreviousBoards, forceSync = false) {
                ...
                return Promise.all(boardLoadPromises).then((loadedIds) => {
                    ...
                    _.each(loadedIds, function (id) {
                        if (id === false) return;
                        let boardDb = profileManager.getJsonDbInProfile("/user-settings/controls/" + id);
            ...
            */
            ...
            service.saveControlForCurrentBoard = function(control) {
                let boardDb = profileManager.getJsonDbInProfile("/user-settings/controls/" + settingsService.getLastBoardId());
                ...
                boards.forEach((board) => {
                    ...
                    scenes.forEach((scene) => {
                        ...
                        if (index !== -1) {
                            ...
                            //save to file
                            let boardDb = profileManager.getJsonDbInProfile(`/user-settings/controls/${board.name}`);
