    ...
    angular
        ...
        .factory("updatesService", function(
            ...
            service.updateIsAvailable = function() {
                return service.hasCheckedForUpdates
                    ? service.updateData.updateIsAvailable
                    : false;
            };
            ...
            service.checkForUpdate = function() {
                ...
                return $q(resolve => {
                    let firebotReleasesUrl =
            "https://api.github.com/repos/Firebottle/Firebot/releases/latest";

                    if (
                        settingsService.notifyOnBeta() ||
            settingsService.getAutoUpdateLevel() >= 4
                    ) {
                        firebotReleasesUrl =
              "https://api.github.com/repos/Firebottle/Firebot/releases";
                    }

                    $http.get(firebotReleasesUrl).then(
                        response => {
                            // Get app version
                            ...
                            let updateType = VersionCompare.compareVersions(
                                gitNewest.tag_name,
                                currentVersion
                            );
                            ...
                            if (updateType !== UpdateType.NONE) {
                                ...
                                if (shouldAutoUpdate(autoUpdateLevel, updateType)) {
                                    ...
                                    listenerService.fireEvent(
                                        listenerService.EventType.DOWNLOAD_UPDATE
                                    );
                                } else {
